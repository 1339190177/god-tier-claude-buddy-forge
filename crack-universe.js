...
if (isMainThread) {
  const { SPECIES, STAT_NAMES } = require('./lib/constants')
  ...
  for (let i = 0; i < numWorkers; i++) {
    ...
    const worker = new Worker(__filename, {
      workerData: {
        phase: 1, startSeed, endSeed,
        TARGET_SPECIES, TARGET_DUMP, SALT, MIN_RARITY, REQUIRE_SHINY,
      },
    })
        ...
        if (stage1Finished === numWorkers) {
          ...
          for (let j = 0; j < numWorkers; j++) {
            const hashWorker = new Worker(__filename, {
              workerData: { phase: 2, targetSeed: bestGlobalSeed, SALT, prefix: `THE_ONE_${Math.random().toString(36).substring(2, 6)}_${j}_` },
            })
  ...
  const { SPECIES, STAT_NAMES, RARITIES, RARITY_WEIGHTS, RARITY_RANK } = require('./lib/constants')
  ...
  if (workerData.phase === 1) {
    ...
    for (let seed = workerData.startSeed; seed <= workerData.endSeed; seed++) {
      ...
      for (let ri = 0; ri < RARITIES.length; ri++) {
        ...
        if (rarityVal < 0) { rarityIdx = ri; break }
      }
