...
const { SPECIES, RARITIES, STAT_NAMES } = require('./constants')
 ...
 */
function parseArgs(argv, extraParams = {}, opts = {}) {
...
function applyParam(result, key, val, params) {
  ...
  result[key] = params[key].type === 'number' ? parseInt(val) || params[key].default : val
}
...
function showHelpAndExit(extraParams = {}, opts = {}) {
  ...
  for (const key of allKeys) {
    ...
    const def = p.default !== undefined && p.default !== false ? ` (默认: ${p.default})` : ''
...
module.exports = { parseArgs, validateOrExit, showHelpAndExit, resolveArgs, GLOBAL_PARAMS }
