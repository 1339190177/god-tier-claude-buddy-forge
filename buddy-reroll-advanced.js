...
console.log('===============================================');
console.log(
  `开始在 ${RUN_SECONDS} 秒内暴力刷取 ${MIN_RARITY}+${REQUIRE_SHINY ? ' 闪光' : ''} ${TARGET_SPECIES}！`,
);
...
while (Date.now() < endTime) {
  ...
  for (let j = 0; j < 50000; j++) {
    ...
    void pick(rng, EYES);
    void (rarity === 'common' ? 'none' : pick(rng, HATS));
    ...
    if (sum > best.sum) {
      ...
      console.log(
        `[新纪录] ${rarity}${shiny ? ' 闪光' : ''} ${species}！总属性=${sum} / UID: ${uid.substring(0, 8)}... (${(loops * 50000 + j) / 10000}万次运算)`,
      );
    }
  }
}
...
console.log('\n===============================================');
console.log(
  `搜索结束！总共运行了 ${Math.floor((loops * 50000) / 10000)} 万次哈希运算。`,
);
if (!best.uid) {
  console.log(
    '运气不佳，在设定时间内未刷到符合条件的个体，请增加时间或放宽条件后重试。',
  );
} else {
  console.log(
    `最佳捕获: ${best.rarity} ${best.shiny ? '✨ Shiny✨ ' : ''}${best.species}`,
  );
  console.log(`隐藏属性面板 (总评: ${best.sum}分，理论极端分421分):`);
  for (const [key, val] of Object.entries(best.stats)) {
    console.log(
      `   - ${key.padEnd(10)}: ${val}  ${val >= 95 ? '<--- Peak' : val <= 54 ? '<--- Dump' : ''}`,
    );
  }
