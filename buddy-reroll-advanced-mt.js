...
if (isMainThread) {
  ...
  console.log(`目标：${MIN_RARITY}+${REQUIRE_SHINY ? ' 闪光' : ''} ${TARGET_SPECIES}`)
  ...
  for (let i = 0; i < numWorkers; i++) {
    const worker = new Worker(__filename, {
      workerData: { runSeconds, TARGET_SPECIES, SALT, MIN_RARITY, REQUIRE_SHINY },
    })
    workers.push(worker)
  ...
  const { runSeconds, TARGET_SPECIES, SALT, MIN_RARITY, REQUIRE_SHINY } = workerData
